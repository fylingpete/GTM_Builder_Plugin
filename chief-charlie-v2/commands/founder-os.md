...
1. **Verify state exists.** Run `ls 01_command_center/dashboards/dashboard_data.json` — if it does not exist, tell the user to run `/onboarding` first and stop.

2. **Trust the SessionStart snapshot** if it's in your context (look for "Founder OS Dashboard Snapshot"). Otherwise read `01_command_center/dashboards/dashboard_data.json` directly.
...
5. **Load the current step's rich content.** Read `01_command_center/dashboards/roadmap_data.json` and pull the entry whose `id` matches `currentPhase` (e.g., `"1.3"`).
